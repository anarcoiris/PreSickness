...
"""
spider_ocr_pipeline_v2.py

Avanzado Spider + ETL para documentos históricos digitalizados (España u otros).
Características principales:
- Crawling asíncrono (aiohttp + asyncio) con respeto básico a robots.txt y rate-limiting por dominio.
- Soporta extracción de texto desde: HTML, TXT, DOCX, PDF (texto si existe) y OCR (páginas convertidas a imágenes).
- OCR paralelo usando process pool (pdf2image + pytesseract).
- Persistencia metadata + deduplicado usando SQLite (aiosqlite).
- Filtrado por idioma (langdetect), tamaño mínimo, heurísticas de calidad OCR y limpieza (ftfy + NFKC).
- Evita re-procesar URLs/archivos ya vistos; mapea contenido por hash (sha256).
- Pipeline configurable: objetivos de búsqueda, expansión de términos, max profundidad, política de exclusión por patrones, rutas de salida.
- Modular: fáciles de extender para APIs específicas (BNE, PARES) si deseas añadir scrapers específicos.

USO (ejemplo):
    python main.py --seeds seeds.txt --outdir ./downloads --db spider_meta.db \
        --max-pages 200 --max-depth 2 --concurrency 8 --ocr-lang spa --min-words 50

DEPENDENCIAS (pip):
    pip install aiohttp aiofiles aiosqlite beautifulsoup4 tqdm ftfy pillow pytesseract pdf2image python-docx langdetect

SISTEMA (instalación externa requerida):
    - Tesseract OCR (con español): instalar y asegurarse que pytesseract lo localice.
    - Poppler (para pdf2image): instalar y, si es Windows, configurar poppler_path o la variable POPPLER_PATH.

NOTA DE RESPONSABILIDAD:
    - Respeta las condiciones de uso y licencias de los repositorios web. Esto es una herramienta técnica; úsala de manera responsable.
"""
...
class Spider:
    ...
    async def seed_urls(self, seeds: List[str]):
        """Insert seeds into DB with depth=0."""
        now = time.time()
    ...
    async def _process_url_task(self, url: str, depth: int):
        ...
        try:
            ...
            if detect is not None:
                try:
                    lang = await self.loop.run_in_executor(None, detect, norm_text[:2000]) if norm_text else None
                except Exception:
                    lang = None
            if lang and lang != "es":
                lang_ok = False
...
def parse_args():
    ...
    p.add_argument("--seeds", required=True, help="Text file with seed URLs (one per line)")
    ...
    p.add_argument("--coninuous", action="store_true", dest="continuous", help="Keep crawling continuously (poll DB)")
    p.add_argument("--concurrency-http", type=int, default=6, help="HTTP concurrency (same as --concurrency)")
...
async def main_async():
    ...
    if not seeds_file.exists():
        ...
        sys.exit(1)
    seeds = [line.strip() for line in seeds_file.read_text(encoding="utf-8").splitlines() if line.strip()]
    config = SpiderConfig(
        outdir=args.outdir,
        db_path=args.db,
        user_agent=DEFAULT_USER_AGENT,
        concurrency=args.concurrency_http,
