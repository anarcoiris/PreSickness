...
def verify_main_py():
    ...
    if not Path('main.py').exists():
        console.print("[red]❌ main.py no encontrado[/]")
        return False
    
    # Verifica que Python pueda importar los módulos necesarios
    try:
        ...
        if result.returncode != 0:
            console.print("[red]❌ Dependencias no disponibles[/]")
            console.print(f"[dim]{result.stderr}[/]")
            return False
    except subprocess.TimeoutExpired:
        console.print("[yellow]⚠️  Verificación de dependencias tardó demasiado[/]")
    except Exception as e:
        console.print(f"[yellow]⚠️  No se pudo verificar dependencias: {e}[/]")
...
def show_menu(title: str, options: List[str], allow_back: bool = True) -> int:
    ...
    Returns:
        Índice de la opción (0-based), o -1 si elige volver
    """
    console.print(f"[bold yellow]{title}[/]\n")
    
    table = Table(show_header=False, box=box.SIMPLE, padding=(0, 2))
    table.add_column("Opción", style="cyan", justify="right")
    ...
    while True:
        try:
            ...
            console.print("[red]❌ Por favor ingresa un número[/]")
...
def step_prepare_corpus(state: PipelineState):
    ...
    if not Path(input_file).exists():
        console.print(f"[red]❌ Archivo no encontrado: {input_file}[/]")
    ...
    preserve_case = Confirm.ask("¿Preservar mayúsculas?", default=False)
    remove_control = Confirm.ask("¿Eliminar caracteres de control?", default=True)
    
    # Confirmar
    console.print("\n[bold yellow]📋 Resumen:[/]")
    console.print(f"  Input:  {input_file}")
    console.print(f"  Output: {output_file}")
    console.print(f"  Preservar mayúsculas: {preserve_case}")
    ...
    # Ejecuta
    if run_command(cmd, "Preparación de corpus"):
...
def step_analyze_corpus(state: PipelineState):
    ...
    show_header(
        "🔍 PASO 2: ANÁLISIS DE CORPUS",
        "Analiza estructura y sugiere hiperparámetros"
    )
    ...
    if not Path(corpus_file).exists():
        console.print(f"[red]❌ Archivo no encontrado: {corpus_file}[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    
    # Separator
    use_separator = Confirm.ask(
        "¿Tu corpus tiene separadores de documento?",
        default=True
    )
    ...
    # Ejecuta
    run_command(cmd, "Análisis de corpus")
    
    # Permite ajustar parámetros basado en análisis
    console.print("\n[bold yellow]💡 Basándote en el análisis, puedes ajustar:[/]")
    
    if Confirm.ask("¿Ajustar parámetros ahora?", default=True):
...
def step_train_tokenizer(state: PipelineState):
    ...
    show_header(
        "🔤 PASO 3: ENTRENAMIENTO DE TOKENIZER",
    ...
    files = [corpus_file]
    if Confirm.ask("¿Agregar más archivos?", default=False):
        while True:
            ...
            if Path(extra).exists():
                ...
                console.print(f"[yellow]⚠️  Archivo no encontrado: {extra}[/]")
...
def step_train_model(state: PipelineState):
    ...
    if not Path(tokenizer).exists():
        console.print(f"[red]❌ Tokenizer no encontrado: {tokenizer}[/]")
        console.print("[yellow]💡 Ejecuta primero el paso 3: Entrenar tokenizer[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    
    if not Path(corpus).exists():
        console.print(f"[red]❌ Corpus no encontrado: {corpus}[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    
    # Configuración de modelo
    console.print("[bold cyan]🏗️  Arquitectura del modelo[/]\n")
    
    model_configs = {
        'small': {
            'desc': 'Pequeño (~5M parámetros, rápido)',
            'info': 'n_embd=256, n_layer=6, n_head=8'
        },
        'medium': {
            'desc': 'Mediano (~15M parámetros, balanceado)',
            'info': 'n_embd=384, n_layer=10, n_head=6'
        },
        'large': {
            'desc': 'Grande (~40M parámetros, mejor calidad)',
            'info': 'n_embd=512, n_layer=12, n_head=8'
        },
        'custom': {
            'desc': 'Personalizado (configurar manualmente)',
            'info': 'Define tus propios hiperparámetros'
        }
    }
    ...
    if config_name == 'custom':
        console.print("\n[bold]Configuración personalizada[/]\n")
    ...
    console.print("\n[bold cyan]⚙️  Configuración de entrenamiento[/]\n")
    ...
    # Opciones avanzadas
    console.print()
    if Confirm.ask("¿Configurar opciones avanzadas?", default=False):
        ...
        use_rope = Confirm.ask("¿Usar RoPE (position embeddings)?", default=True)
        use_amp = Confirm.ask("¿Usar mixed precision (AMP)?", default=True)
    else:
        weight_decay = 0.01
        warmup_steps = 500
        grad_clip = 1.0
    ...
    if separator:
        use_doc_aware = Confirm.ask(
            "¿Usar dataset document-aware? (recomendado)",
            default=True
        )
    else:
        use_doc_aware = False
    ...
    # Resume
    resume_from = None
    if Confirm.ask("¿Continuar desde checkpoint existente?", default=False):
        resume_from = Prompt.ask("Ruta al checkpoint")
        if not Path(resume_from).exists():
            console.print("[yellow]⚠️  Checkpoint no encontrado, comenzando desde cero[/]")
            resume_from = None
    
    # Resumen
    console.print("\n" + "="*70)
    console.print("[bold yellow]📋 RESUMEN DE CONFIGURACIÓN[/]")
    console.print("="*70)
    ...
    if not Confirm.ask("¿Iniciar entrenamiento?", default=True):
        return
...
def step_generate(state: PipelineState):
    ...
    if not tokenizer or not Path(tokenizer).exists():
        console.print(f"[red]❌ Tokenizer no encontrado[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    ...
    if not Path(checkpoint).exists():
        console.print(f"[red]❌ Checkpoint no encontrado: {checkpoint}[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    
    # Modo de generación
    console.print("\n[bold]Modo de generación[/]\n")
    mode = show_menu(
        "Selecciona modo",
        [
            "Prompt único (interactivo)",
    ...
    console.print("\n[bold cyan]⚙️  Parámetros de generación[/]\n")
    
    max_tokens = IntPrompt.ask("Tokens máximos a generar", default=100)
    ...
    if Confirm.ask("¿Configurar opciones avanzadas?", default=False):
        ...
        greedy = Confirm.ask("¿Usar greedy decoding? (ignora temperature)", default=False)
        stream = Confirm.ask("¿Mostrar tokens en tiempo real (streaming)?", default=False)
    else:
    ...
    # Según modo
    if mode == 0:  # Prompt único
        prompt = Prompt.ask("\nIngresa tu prompt")
        
        cmd = cmd_base + ['--prompt', prompt, '--stats']
        
        output_file = Prompt.ask(
            "¿Guardar en archivo? (Enter para no guardar)",
            default=""
        )
        if output_file:
            cmd.extend(['--output', output_file])
        
        run_command(cmd, "Generación de texto")
    
    elif mode == 1:  # Múltiples prompts
        prompt_file = Prompt.ask("Archivo con prompts (uno por línea)")
        
        if not Path(prompt_file).exists():
            console.print(f"[red]❌ Archivo no encontrado: {prompt_file}[/]")
            Prompt.ask("\nPresiona Enter para continuar")
            return
        ...
        output_file = Prompt.ask(
            "¿Guardar resultados? (Enter para no guardar)",
            default=""
        )
        if output_file:
            cmd.extend(['--output', output_file])
        
        run_command(cmd, "Generación múltiple")
        ...
        run_command(cmd, "Generación batch")
...
def step_evaluate(state: PipelineState):
    ...
    show_header(
        "📊 PASO 6: EVALUACIÓN DE MODELO",
        "Calcula perplexity y otras métricas"
    )
    ...
    mode = show_menu(
        "Selecciona fuente",
        [
            "Archivo con múltiples textos",
            "Texto único (escribir ahora)"
        ],
        allow_back=True
    )
    ...
    if mode == 0:  # Archivo
        ...
        if not Path(test_file).exists():
            console.print(f"[red]❌ Archivo no encontrado: {test_file}[/]")
            Prompt.ask("\nPresiona Enter para continuar")
            return
        
        cmd.extend(['--test-file', test_file])
    
    elif mode == 1:  # Texto único
        test_text = Prompt.ask("Texto a evaluar")
        cmd.extend(['--test-text', test_text])
    
    # Ejecuta
    run_command(cmd, "Evaluación de modelo")
...
def main_menu(state: PipelineState):
    ...
    while True:
        show_header(
            "🚀 MINI-LLM - PIPELINE INTERACTIVO",
            "CLI guiado para entrenar modelos de lenguaje"
        )
        
        # Mostrar estado actual
        console.print("[bold]📁 Estado del proyecto:[/]\n")
        ...
        corpus_status = "✅" if corpus_clean and Path(corpus_clean).exists() else "❌"
        ...
        # Tokenizer
        tokenizer = state.get('tokenizer')
        tok_status = "✅" if tokenizer and Path(tokenizer).exists() else "❌"
        status_table.add_row(
            "Tokenizer",
        ...
        # Modelo
        checkpoint = state.get('last_checkpoint')
        model_status = "✅" if checkpoint and Path(checkpoint).exists() else "❌"
        status_table.add_row(
            "Modelo entrenado",
        ...
        # Menú de opciones
        options = [
            "📝 Preparar corpus (limpieza y normalización)",
            "🔍 Analizar corpus (estadísticas y sugerencias)",
            "🔤 Entrenar tokenizer (BPE)",
            "🤖 Entrenar modelo",
            "✨ Generar texto",
            "📊 Evaluar modelo (perplexity)",
        ...
        choice = show_menu("¿Qué deseas hacer?", options, allow_back=False)
...
def advanced_config(state: PipelineState):
    ...
    show_header(
        "⚙️  CONFIGURACIÓN AVANZADA",
        "Ajusta parámetros por defecto del pipeline"
    )
    
    console.print("[bold yellow]Configuración actual:[/]\n")
    ...
    if Confirm.ask("¿Modificar configuración?", default=True):
        ...
        console.print("\n[green]✅ Configuración actualizada[/]")
...
def show_help():
    ...
    help_text = """
[bold cyan]🎯 WORKFLOW RECOMENDADO:[/]

[bold]1. Preparar corpus[/]
   - Limpia y normaliza tu archivo de texto
   - Elimina caracteres problemáticos
   - Opcional: convierte a minúsculas

[bold]2. Analizar corpus[/]
   - Obtén estadísticas sobre tus datos
   - Recibe sugerencias de hiperparámetros
   - Decide block_size y vocab_size óptimos

[bold]3. Entrenar tokenizer[/]
   - Crea un tokenizer BPE específico para tu corpus
   - Define el tamaño de vocabulario (16k-64k típicamente)
   - Incluye tokens especiales si los necesitas

[bold]4. Entrenar modelo[/]
   - Elige arquitectura (small/medium/large/custom)
   - Configura hiperparámetros (epochs, batch_size, lr)
   - El entrenamiento guarda checkpoints automáticamente
...
• [bold]Corpus pequeño (<50MB):[/] usa modelo 'small', block_size=128-256
• [bold]Corpus mediano (50-200MB):[/] usa 'medium', block_size=256-512
• [bold]Corpus grande (>200MB):[/] usa 'large', block_size=512-1024

• [bold]Separadores de documento:[/] mejoran la calidad si tu corpus
  tiene documentos claramente separados

• [bold]Temperature:[/]
  - 0.1-0.5: más determinístico y coherente
  - 0.8-1.0: balanceado (recomendado)
  - 1.5-2.0: más creativo y aleatorio

• [bold]Early stopping:[/] el entrenamiento para automáticamente
  si no hay mejora en 5 epochs

[bold cyan]🔧 ARCHIVOS GENERADOS:[/]

• [bold]tokenizer.json[/]: vocabulario y reglas de tokenización
• [bold]runs/experiment/[/]:
...
[bold cyan]⚠️  SOLUCIÓN DE PROBLEMAS:[/]

• [bold]"Weights only load failed"[/]:
  Asegúrate de haber ejecutado fix_torch_load.py

• [bold]"CUDA out of memory"[/]:
  - Reduce batch_size
...
• [bold]"Loss is NaN"[/]:
  ...
  - Verifica que el corpus esté bien formateado
  - Usa gradient clipping (default: 1.0)

• [bold]Generación repetitiva[/]:
  - Aumenta temperature (prueba 1.0-1.2)
  - Aumenta repetition_penalty (prueba 1.1-1.3)
  - Usa top-p sampling con valores más bajos (0.8-0.9)
"""
...
def run_full_pipeline(state: PipelineState):
    ...
    console.print("[bold yellow]⚠️  Este proceso ejecutará:[/]\n")
    ...
    console.print()
    console.print("[dim]Nota: Puedes configurar parámetros antes de comenzar[/]\n")
    
    if not Confirm.ask("¿Continuar con pipeline automático?", default=True):
        return
    
    # Configuración inicial
    console.print("\n[bold cyan]📋 Configuración inicial[/]\n")
    ...
    if not Path(corpus_raw).exists():
        console.print(f"[red]❌ Archivo no encontrado: {corpus_raw}[/]")
        Prompt.ask("\nPresiona Enter para continuar")
        return
    ...
    # Confirmar
    console.print(f"\n[bold yellow]📋 Configuración seleccionada ({preset['name']}):[/]")
    ...
    if not Confirm.ask("¿Iniciar pipeline?", default=True):
        return
    
    # Ejecutar pipeline
    console.print("\n" + "="*70)
    console.print("[bold cyan]🚀 INICIANDO PIPELINE AUTOMÁTICO[/]")
    console.print("="*70 + "\n")
    ...
    try:
        # Paso 1: Preparar corpus
        console.print(f"[bold cyan]📝 Paso 1/{total_steps}: Preparando corpus...[/]")
        cmd = [
            sys.executable, 'main.py', 'prepare-corpus',
            '--input', corpus_raw,
            '--output', str(corpus_clean)
        ]
        if not run_command(cmd, "Preparación de corpus"):
            raise Exception("Falló preparación de corpus")
        steps_completed += 1
        
        # Paso 2: Analizar corpus
        console.print(f"\n[bold cyan]🔍 Paso 2/{total_steps}: Analizando corpus...[/]")
        cmd = [
            sys.executable, 'main.py', 'analyze-corpus',
        ...
        # Paso 3: Entrenar tokenizer
        console.print(f"\n[bold cyan]🔤 Paso 3/{total_steps}: Entrenando tokenizer...[/]")
        cmd = [
            sys.executable, 'main.py', 'init-tokenizer',
        ...
        if not run_command(cmd, "Entrenamiento de tokenizer"):
            raise Exception("Falló entrenamiento de tokenizer")
        steps_completed += 1
        
        # Paso 4: Entrenar modelo
        console.print(f"\n[bold cyan]🤖 Paso 4/{total_steps}: Entrenando modelo...[/]")
        console.print("[yellow]⏱️  Este paso puede tardar bastante tiempo...[/]\n")
        cmd = [
            sys.executable, 'main.py', 'train',
        ...
        if not run_command(cmd, "Entrenamiento de modelo"):
            raise Exception("Falló entrenamiento de modelo")
        steps_completed += 1
        
        # Paso 5: Generar ejemplos
        console.print(f"\n[bold cyan]✨ Paso 5/{total_steps}: Generando ejemplos...[/]")
        ...
        for i, prompt in enumerate(test_prompts, 1):
            ...
            run_command(cmd, f"Generación ejemplo {i}")
        
        steps_completed += 1
        
        # Éxito
        console.print("\n" + "="*70)
        console.print("[bold green]✅ PIPELINE COMPLETADO EXITOSAMENTE[/]")
        console.print("="*70)
        console.print(f"\n[bold cyan]📁 Archivos generados en:[/] {output_base}")
        console.print(f"\n[bold]Próximos pasos:[/]")
        console.print("  • Genera más texto con diferentes prompts")
        console.print("  • Evalúa el modelo con textos de test")
        console.print("  • Ajusta hiperparámetros y re-entrena si es necesario")
        console.print()
        ...
        console.print("\n" + "="*70)
        console.print("[bold red]❌ PIPELINE INTERRUMPIDO[/]")
        console.print("="*70)
...
def clean_config(state: PipelineState):
    ...
    show_header(
        "🗑️  LIMPIAR CONFIGURACIÓN",
        "Resetea la configuración guardada"
    )
    
    console.print("[bold yellow]⚠️  Esto borrará:[/]\n")
    console.print("  • Rutas de archivos guardadas")
    console.print("  • Configuración personalizada")
    console.print("  • Historial de experimentos")
    console.print()
    console.print("[dim]Nota: No borra archivos de datos o modelos[/]\n")
    
    if Confirm.ask("¿Confirmar limpieza?", default=False):
...
def main():
    ...
    try:
        ...
        if not Path('main.py').exists():
            console.print("[bold red]❌ Error: main.py no encontrado[/]")
        ...
        console.print()
        console.print(Panel.fit(
            "[bold cyan]🚀 MINI-LLM - PIPELINE INTERACTIVO[/]\n"
        ...
        if Confirm.ask("¿Ver tutorial rápido?", default=False):
            show_help()
